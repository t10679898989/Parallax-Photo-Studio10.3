...
export class AppComponent {
  ...
  setupNativeBridge() {
    ...
    (window as any).handleDoubleTap = () => {
      ...
      // Logic: Trigger Wallpaper Change
      // In a real LWP, the native side might handle the image swap, 
      // but if the WebApp manages the playlist logic, we trigger it here.
      console.log('Double Tap Detected: Requesting Next Wallpaper');
      
      // If we are in "Editor/Preview" mode or passing logic to Native, 
      // we can emit an event or call a service method here.
